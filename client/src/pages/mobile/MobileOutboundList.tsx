...
export default function MobileOutboundList() {
  const navigate = useNavigate();

  const { data: orders, isLoading } = useQuery({
    queryKey: ['outbound'],
    queryFn: () => apiClient.get('/outbound', { params: { pageSize: 50 } }).then(r => r.data.data as OutboundOrder[]),
  });
  ...
  return (
    <div>
      <Typography.Title level={5} style={{ margin: '0 0 12px' }}>出库记录</Typography.Title>

      {orders?.length === 0 && (
        <div style={{ textAlign: 'center', padding: 40, color: '#999' }}>暂无出库记录</div>
      )}

      <Space orientation="vertical" style={{ width: '100%' }} size={8}>
        {orders?.map(order => (
          <Card key={order.id} size="small" style={{ borderRadius: 8, cursor: 'pointer' }}
            onClick={() => navigate(`/m/outbound/${order.id}`)}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <Typography.Text strong>{order.orderNo}</Typography.Text>
                <div style={{ fontSize: 13, color: '#666', marginTop: 2 }}>
                  {order.receiver || '未填写领用人'} · {order.items?.length || 0} 项
                </div>
      ...
      <div style={{ position: 'fixed', bottom: 80, right: 16, zIndex: 50 }}>
        <Button type="primary" shape="round" size="large" icon={<CameraOutlined />}
          onClick={() => navigate('/m/outbound/new')} style={{ height: 48, fontSize: 16 }}>
          扫码出库
        </Button>
      </div>
    </div>
